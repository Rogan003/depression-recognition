...
# NLTK resources needed for lemmatization (downloaded once, then cached).
...
# Number of most-informative words (by TF-IDF) we keep per transcript before
# handing the text to the embedding model.
TOP_K_WORDS = 200


def lemma_tokenizer(text):
    # Lowercase, tokenize, keep alphabetic tokens, and lemmatize each one so
    # that grammatical variants (feel/feels/feeling) collapse to one feature.
...
def build_reduced_texts(texts, top_k=TOP_K_WORDS):
    # Fit a lemmatizing TF-IDF over the corpus, then for each transcript keep
    # only its top-k highest TF-IDF lemmas. This denoises the very long, chatty
    # interview transcripts down to their most informative vocabulary before we
    # embed them with the (length-limited) sentence transformer.
    ...
    for i in range(tfidf.shape[0]):
        ...
        if row.nnz == 0:
            reduced.append("")
            continue
        # Indices of this document's terms sorted by descending TF-IDF weight.
        data = row.data
        cols = row.indices
...
def plot_predictions(y_true, y_pred, model_name, out_path):
    ...
    plt.scatter(x, y_pred_sorted, alpha=0.8, color='seagreen', edgecolors='k',
...
def main():
    ...
    # Lemmatize + keep only the top-K TF-IDF words per transcript. We fit the
    # TF-IDF on the combined corpus so the vocabulary/IDF is shared.
    ...
    print("\n--- Cross-Validation Summary (mpnet + lemma + TF-IDF top words) ---")
    ...
    plot_predictions(
        y_train_dev,
        oof_preds,
        f"mpnet+lemma+TFIDF + {best_result['name']}",
        '../media/mpnet_lemma_tfidf_best_model_predictions.png'
    )
