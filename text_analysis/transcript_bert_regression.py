...
# ---------------------------------------------------------------------------
# Overlapping-window chunking (same simple, elegant scheme as the base
# transformer script `transcript_transformer_regression.py`)
# ---------------------------------------------------------------------------
# A clinical interview is thousands of words, but BERT only sees 512 tokens at
# once. To actually look at the WHOLE interview we split each transcript into
# overlapping word windows, run BERT on every window, mean-pool the token
# embeddings into one window vector, and finally mean-pool the window vectors
# into a single document embedding. The overlap keeps context from being cut
# mid-thought between windows.
CHUNK_WORDS = 200       # words per window (BERT handles 512 tokens, so this is safe)
CHUNK_OVERLAP = 50      # overlap so context isn't cut mid-thought between windows
...
# ---------------------------------------------------------------------------
# Custom simple BERT feature extractor
# ---------------------------------------------------------------------------
class BertFeatureExtractor:
    ...
    def _embed_chunks(self, chunks):
        ...
        for i in range(0, len(chunks), self.batch_size):
            ...
            out = self.model(**enc)
            # Mean-pool the last hidden states using the attention mask so that
            # padding tokens don't dilute the representation.
    ...
    def encode(self, texts, show_progress_bar=True):
        ...
        for idx, text in enumerate(texts):
            ...
            # Mean-pool chunk embeddings into a single document embedding.
...
def main():
    ...
    print("\n--- Cross-Validation Summary on Combined Train+Dev (BERT) ---")
    ...
    plot_predictions(
        y_train_dev,
        oof_preds,
        f"BERT + {best_result['name']}",
        '../media/bert_best_model_predictions.png'
    )
